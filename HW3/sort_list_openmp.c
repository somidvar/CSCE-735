...
int binary_search_lt(int v, int *list, int first, int last) {
    // Linear search code
    // int idx = first; while ((v > list[idx]) && (idx < last)) idx++; return
    // idx;
...
#pragma omp parallel for default(none) num_threads(num_threads) \
    shared(np, num_threads, ptr) private(my_id)
...
// Sort local lists
#pragma omp parallel for default(none) num_threads(num_threads) \
    shared(list, ptr, num_threads) private(my_id, my_list_size)
    for (my_id = 0; my_id < num_threads; my_id++) {
...
#pragma omp parallel for num_threads(num_threads) default(none) shared( \
    list, ptr, level, np, num_threads, list_size, q, work) private(i, my_id)
        for (my_id = 0; my_id < num_threads; my_id++) {
            ...
            if (my_search_blk > my_own_blk) {
                idx = binary_search_lt(list[ptr[my_id]], list, my_search_idx,
                                       my_search_idx_max);
            } else {
                idx = binary_search_le(list[ptr[my_id]], list, my_search_idx,
                                       my_search_idx_max);
            }
            my_search_count = idx - my_search_idx;
            i_write =
                my_write_idx + my_search_count + (ptr[my_id] - my_own_idx);
            ...
            for (i = ptr[my_id] + 1; i < ptr[my_id + 1]; i++) {
                if (my_search_blk > my_own_blk) {
                    ...
                    while ((list[i] >= list[idx]) &&
                           (idx < my_search_idx_max)) {
                        idx++;
                        my_search_count++;
                    }
                }
...
#pragma omp parallel for default(none) \
    shared(ptr, num_threads, list, work) private(i, my_id)
        for (my_id = 0; my_id < num_threads; my_id++) {
...
// Main program - set up list of random integers and use threads to sort the
// list
//
// Input:
...
int main(int argc, char *argv[]) {
    ...
    // Read input, validate
    if (argc != 3) {
        printf("Need two+two integers as input \n");
        printf(
            "Use: <executable_name> <log_2(list_size)> <log_2(num_threads)>\n");
        exit(0);
    }
    k = atoi(argv[argc - 2]);
    ...
    // Print time taken
    printf(
        "List Size = %d, Threads = %d, error = %d, time (sec) = %8.4f, "
        "qsort_time = %8.4f\n",
