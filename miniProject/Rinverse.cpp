...
void RMatrix::compute_matrix_inverse_recursively(double **R, int start, int size)
{
    ...
    {
#pragma omp task // shared(R) //private(start, size)
        // Set R11 = inv(R11)
...
void RMatrix::compute_off_diagonal_block(double **R, int start, int size)
{
    double sum;
    int i, j, k;
#pragma omp parallel for shared(R, Rtemp, start, size) collapse(2) private(i, j, k, sum)
    // Rtemp = -inv(R11)*R12
    for (i = start; i < start + size; i++)
    {
        for (j = start + size; j < start + 2 * size; j++)
        {
            sum = 0.0;
...
// R12 = Rtemp*inv(R22)
#pragma omp parallel for private(i, j, k, sum) collapse(2) shared(R, Rtemp, start, size)
    for (i = start; i < start + size; i++)
    {
        for (j = start + size; j < start + 2 * size; j++)
        {
            sum = 0.0;
            for (k = start + size; k < start + 2 * size; k++)
                sum += Rtemp[i][k] * R[k][j];
            R[i][j] = sum;
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    int true_threads, n_threads = 0;

    if (k <= 6)
        n_threads = round(sqrt(matrix_size));
    else
        n_threads = round(sqrt(matrix_size / 2));
